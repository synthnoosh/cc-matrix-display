...
A physical desk dashboard for Claude Code — drives a 64x32 RGB LED matrix via Adafruit Matrix Portal S3.
...
3. **`matrix/code.py`** — CircuitPython for the Matrix Portal S3. Polls the server over WiFi, renders to the 64x32 LED panel.

## Conventions

- **Host code**: Python 3.9+ stdlib only. No pip dependencies. Single-file server.
- **Matrix code**: CircuitPython 9.x targeting ESP32-S3 (Matrix Portal S3). Libraries from the Adafruit CircuitPython bundle.
